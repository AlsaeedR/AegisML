...
def dispatch_sandbox(
    ...
    max_oom_retries: Optional[int] = None,
) -> Dict[str, Any]:
    """
    ...
    When audit_id is provided, real-time telemetry events (container start,
    periodic CPU/memory stats, completion) are published to the shared
    telemetry_bus so api.py can stream them to a connected client via SSE.
    """
    ...
    if not docker_ready and not allow_insecure:
        ...
        publish_telemetry(audit_id, {
            "event": "sandbox_skipped",
            "reason": "docker_unavailable",
        })
        result = _build_fail_closed_skip_response(
    ...
    if docker_ready:
        result = _run_in_docker(
            ...
            max_oom_retries=(
                max_oom_retries
                if max_oom_retries is not None
                else int(os.getenv("AEGISML_MAX_OOM_RETRIES", "1"))
            ),
        )
        close_stream(audit_id)
        return result
...
def _poll_container_stats(container_name: str, audit_id: Optional[str], stop_event: threading.Event, memory_limit_bytes: int, interval_seconds: float = 1.5) -> None:
    """
    Background thread target: polls `docker stats` for the running container
    every `interval_seconds` and publishes a telemetry snapshot until the
    container finishes (stop_event is set) or stats can no longer be read
    (container already exited).

    Also acts as a lightweight watchdog: if memory usage crosses 85% of the
    container's configured limit, publishes an early "memory_warning" event
    so the caller/UI can surface risk before an actual OOM kill happens.
    """
    warned = False
            ...
            if res.returncode == 0 and res.stdout.strip():
                ...
                if not warned and mem_pct >= 85.0:
                    ...
                    publish_telemetry(audit_id, {
                        ...
                        "mem_percent": mem_pct,
                        "message": (
                            "Container memory usage exceeded 85% of its limit. "
                            "An OOM kill may occur; adaptive downscaling will "
                            "trigger automatically on retry if the container is killed."
                        ),
                    })
        except (subprocess.TimeoutExpired, json.JSONDecodeError, Exception):
            # Container may have already exited between the loop check and
            # the stats call - this is expected near the end of execution,
            # not an error worth surfacing to the client.
            pass
...
def _was_oom_killed(container_name: str) -> bool:
    """
    Inspects a stopped (but not yet removed) container to determine whether
    it was killed by the OOM killer. Requires the container to have been run
    WITHOUT --rm so it still exists at inspection time.
    """
    try:
        res = subprocess.run(
...
def _downscale_strategy_config(strategy_config: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Produces a reduced-workload copy of the strategy config for a retry
    attempt after an OOM kill: halves the adversarial sample size (floor 10)
    and drops the two lightest-priority static checks (V2/V3) from the
    re-attempted test set, keeping only the higher-severity model-level
    tests (V1/V4) that are most likely to have caused the memory spike.
    """
...
def _run_in_docker(
    ...
    Streams real-time container telemetry (CPU/memory/network usage) to the
    telemetry_bus while the container runs. If the container is killed by
    the OOM killer, automatically retries once (by default) with a
    downscaled attack configuration (smaller adversarial sample size) so a
    single oversized test does not fail the entire audit outright.
    """
    ...
    while True:
        ...
        # NOTE: --rm is intentionally omitted so the container can still be
        # inspected for OOMKilled status after it stops. The container is
        # removed explicitly in the `finally` block of each attempt instead.
        docker_cmd = [
            "docker", "run",
            "--name", container_name,
            "--network", "none",
        ...
        stats_thread = threading.Thread(
            target=_poll_container_stats,
            args=(container_name, audit_id, stop_stats_event, memory_limit_bytes),
            daemon=True,
        )
        ...
        try:
            ...
            if oom_killed and attempt <= max_oom_retries:
                publish_telemetry(audit_id, {
                    "event": "oom_detected_retrying",
                    "container_id": container_name,
                    "attempt": attempt,
                    "message": (
                        "Container was killed by the OOM killer. Retrying "
                        "with a downscaled attack configuration "
                        "(reduced adversarial sample size)."
                    ),
                })
                current_strategy_config = _downscale_strategy_config(current_strategy_config)
                _remove_container(container_name)
                shutil.rmtree(temp_dir, ignore_errors=True)
                continue  # retry the while loop with the downscaled config
            ...
            error_reason = (
                "Container was killed by the OOM killer and the retry budget "
                f"({max_oom_retries}) was exhausted."
                if oom_killed else
                f"Container finished without producing output. Exit code: {returncode}. Stderr: {stderr}"
            )
            publish_telemetry(audit_id, {
            ...
            return {
                ...
                "error": error_reason,
                "telemetry": {"exit_code": returncode, "stderr": stderr, "oom_killed": oom_killed, "attempts": attempt},
            }
...
def _parse_memory_limit(limit_str: str) -> int:
    """Converts a Docker memory limit string (e.g. '4g', '512m') to bytes."""
...
def _run_insecure_local(
    ...
    # If the vectorizer was saved separately from the classifier, combine
    # them into a single sklearn Pipeline so downstream tests (which call
    # model.fit(X_text, y) / model.predict(X_text) on raw text) work
    # correctly regardless of how the artifacts were saved.
    ...
    test_state = {
        ...
        "adversarial_config": strategy_config,
    }
