...
def run_adversarial_test(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if preprocessor is None:
        return {'adversarial_evidence': {'vulnerability_id': 'V4', 'vulnerability_name': 'Adversarial Robustness', 'status': 'inconclusive', 'severity': 'low', 'evidence': {'status': 'skipped', 'reason': 'Could not extract or resolve a preprocessing step to produce numeric feature vectors.'}}}
    try:
        from art.estimators.classification import SklearnClassifier
        from art.attacks.evasion import HopSkipJump
    except ImportError:
        return {'adversarial_evidence': {'vulnerability_id': 'V4', 'vulnerability_name': 'Adversarial Robustness', 'status': 'inconclusive', 'severity': 'low', 'evidence': {'status': 'skipped', 'reason': 'adversarial-robustness-toolbox (ART) is not installed in the environment.'}}}
    try:
        ...
        relative_perturbations[valid_mask] = perturbation_norms[valid_mask] / original_norms[valid_mask]
        ...
        avg_relative_perturbation = float(np.mean(relative_perturbations[valid_mask])) if np.any(valid_mask) else None
        status = 'vulnerable' if success_rate_within_budget >= 0.3 else 'not_vulnerable'
        severity = 'high' if success_rate_within_budget >= 0.6 else 'medium' if success_rate_within_budget >= 0.3 else 'low'
        return {'adversarial_evidence': {'vulnerability_id': 'V4', 'vulnerability_name': 'Adversarial Robustness', 'status': status, 'severity': severity, 'evidence': {'method': 'art_hopskipjump_evasion', 'n_samples_tested': sample_size, 'n_classes': n_classes, 'flipped_predictions_any_perturbation': flipped, 'success_rate_any_perturbation': success_rate, 'max_relative_perturbation_budget': max_relative_perturbation, 'flipped_predictions_within_budget': flipped_within_budget, 'attack_success_rate_within_budget': success_rate_within_budget, 'avg_perturbation_norm': round(avg_perturbation, 4), 'avg_relative_perturbation': round(avg_relative_perturbation, 4) if avg_relative_perturbation is not None else None, 'n_near_zero_vector_samples_excluded': n_zero_norm_samples, 'interpretation': 'A significant portion of evaluation samples were flipped within a realistic perturbation budget, confirming model susceptibility to adversarial evasion.' if success_rate_within_budget >= 0.3 else 'Attacks required unrealistically large perturbations; the model proved empirically robust within normal perturbation boundaries.'}}}
